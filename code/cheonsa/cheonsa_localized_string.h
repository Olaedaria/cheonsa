...
namespace cheonsa
{
	...
	class localized_string_c
	{
	public:
		// a reference to a string instance.
		// these instances are persistent.
		// but they reference a localized_string_c instance which may be volatile.
		class reference_c
		{
		private:
			friend class content_manager_c;

			static core_linked_list_c< reference_c * > _global_list; // all localized_string_c::reference_c instances in existence.
			core_linked_list_c< reference_c * >::node_c _global_list_node; // this instance's list node in _global_instance_list.

			string8_c _key;
			localized_string_c const * _value;
		...
		public:
			...
			void_c set( localized_string_c const * value ); // a little more optimal than set_key() since it avoids doing a look up.

			string8_c const & get_key() const;
			void_c set_key( string8_c const & value ); // sets the key and then tries to resolve value with it.

			localized_string_c const * get_value() const;
	...
	private:
		friend class localized_string_file_c;
	...
	public:
		...
		localized_string_c & operator = ( localized_string_c const & other ); // needed so that the _string_heap can be resized.
