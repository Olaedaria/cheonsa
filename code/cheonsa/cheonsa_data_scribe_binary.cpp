...
namespace cheonsa
{
	...
	{
		assert( _stream && size > 0 && size <= flip_buffer_size );
	...
	boolean_c data_scribe_binary_c::save_string16( string16_c const & value )
	{
		...
		{
			string16_c flipped_string = string16_c( value.character_list.get_internal_array(), core_list_mode_e_dynamic, value.character_list.get_length() );
