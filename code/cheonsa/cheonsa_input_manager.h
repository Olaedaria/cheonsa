...
namespace cheonsa
{
	...
	private:
		...
		boolean_c _event_snapshot_modifier_keys_state[ input_modifier_key_e_count_ ]; // "living" modifier keys state snapshot, updated for each keyboard key event that occurs.
		vector32x2_c _event_snapshot_mouse_position; // "living" mouse position state snapshot, updated for each mouse move event that occurs.
		boolean_c _event_snapshot_mouse_keys_state[ input_mouse_key_e_count_ ]; // "living" mouse keys state snapshot, updated for each mouse key event that occurs.
		boolean_c _event_snapshot_keyboard_keys_state[ input_keyboard_key_e_count_ ]; // "living" keyboard keys state snapshot, updated for each keyboard key event that occurs.
		boolean_c _event_snapshot_gamepad_keys_state[ input_gamepad_key_e_count_ ]; // "living" gamepad keys state snapshot, updated for each gamepad key event that occurs.

		boolean_c _frame_snapshot_modifier_keys_state[ input_modifier_key_e_count_ ]; // modifier key states at start of current frame.
		vector32x2_c _frame_snapshot_mouse_position_last; // position of mouse at start of last frame.
		vector32x2_c _frame_snapshot_mouse_position; // position of mouse at start of current frame.
		vector32x2_c _frame_snapshot_mouse_position_delta; // change in position between start of last frame and start of current frame.
		boolean_c _frame_snapshot_mouse_keys_state[ input_mouse_key_e_count_ ]; // state of mouse keys at start of current frame.
		boolean_c _frame_snapshot_keyboard_keys_state[ input_keyboard_key_e_count_ ]; // state of keyboard keys at start of current frame.
		input_gamepad_state_c _frame_snapshot_gamepad_state; // state of the gamepad at start of current frame.
	...
	public:
		// creates a new action with the given key and returns it.
		// the game should add all of the actions that it will ever need to the input manager at start up.
		...
		// if the given input event matches an action, then invokes that action.
		//input_action_c * _invoke_action_with_input_event( input_event_c * input_event );
