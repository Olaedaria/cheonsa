...
namespace cheonsa
{
	...
	boolean_c database_table_c::_load( data_scribe_binary_c & scribe )
	{
		...
		for ( sint32_c i = 0; i < strings_offsets.get_length(); i++ )
		{
			...
			string8_c string_value = string8_c( &_string_table[ string_offset ], core_list_mode_e_dynamic );
	...
	sint32_c database_table_c::_add_string8( string8_c const & value )
	{
		...
		_string_table_map.insert( string8_c( value.character_list.get_internal_array(), core_list_mode_e_dynamic, value.character_list.get_length() ), strings_buffer_offset );
		return strings_buffer_offset;
	}

	string8_c database_table_c::_get_string8( sint32_c offset ) const
	{
		return string8_c( &_string_table[ offset ], core_list_mode_e_static_volatile );
	}
					...
					for ( sint32_c j = 0; j < remaps.get_length(); j++ )
					{
						...
						if ( remap->from )
						{
							...
							if ( old_field_schema._type == data_type_e_string8 || old_field_schema._type == data_type_e_string16 )
							{
								...
								*reinterpret_cast< sint32_c * >( &new_record_data[ remap->to->_data_offset + 1 ] ) = _add_string8( string8_c( &old_string_table[ *reinterpret_cast< sint32_c const * >( old_record_data[ remap->from->_data_offset + 1 ] ) ], core_list_mode_e_static ) );
							}
							else
							{
								// straight copy.
