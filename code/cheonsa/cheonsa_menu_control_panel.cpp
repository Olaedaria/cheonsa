...
namespace cheonsa
{
	...
	void_c menu_control_panel_c::_handle_scroll_on_value_changed( menu_control_scroll_c * scroll )
	{
		if ( scroll == _horizontal_scroll )
		{
			_client->set_content_offset_horizontal( static_cast< float32_c >( -_horizontal_scroll->get_value() ) );
		}
		else if ( scroll == _vertical_scroll )
		{
			_client->set_content_offset_vertical( static_cast< float32_c >( -_vertical_scroll->get_value() ) );
		}
	}
	...
	void_c menu_control_panel_c::set_horizontal_scroll_visibility_mode( menu_visibility_mode_e value )
	{
		...
		if ( value == menu_visibility_mode_e_always || value == menu_visibility_mode_e_automatic )
		{
			if ( _horizontal_scroll == nullptr )
			{
				...
				_horizontal_scroll->on_value_changed_commit.subscribe( this, &menu_control_panel_c::_handle_scroll_on_value_changed );
		...
		if ( _horizontal_scroll != nullptr )
		{
			_horizontal_scroll->set_value_range_and_page_size( 0.0, _client->get_content_box().get_width(), _client->get_local_box().get_width() );
	...
	void_c menu_control_panel_c::set_vertical_scroll_visibility_mode( menu_visibility_mode_e value )
	{
		...
		if ( value == menu_visibility_mode_e_always || value == menu_visibility_mode_e_automatic )
		{
			if ( _vertical_scroll == nullptr )
			{
				...
				_vertical_scroll->on_value_changed_commit.subscribe( this, &menu_control_panel_c::_handle_scroll_on_value_changed );
		...
		if ( _vertical_scroll != nullptr )
		{
			_vertical_scroll->set_value_range_and_page_size( 0.0, _client->get_content_box().get_height(), _client->get_local_box().get_height() );
