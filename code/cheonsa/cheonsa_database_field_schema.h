...
namespace cheonsa
{
	...
	{
	public:
		database_record_schema_c * record_schema; // the record schema that owns this field.
		string8_c name; // name of this field.
		data_type_e type; // the base type.
		uint8_c type_count; // a value between 1 and 4 that describes the number of elements. should always be 1 for string and list types.

		// used with property inspector user interface to tell it how data can be edited.
		// this data has to be set up by the game at run time, it is not saved|loaded with the database file.
		struct reflection_info_c
		{
