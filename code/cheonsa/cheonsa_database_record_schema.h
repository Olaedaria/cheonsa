...
namespace cheonsa
{
	...
	public:
		...
		database_field_schema_c * add_field( string8_c const & key, data_type_e type, uint8_c type_count ); // add field schema, without reflection info.
		database_field_schema_c const * get_field( string8_c const & key ) const; // returns the field that matches name, otherwise returns nullptr.
		database_field_schema_c const * get_field( string8_c const & key, data_type_e type, uint8_c type_count ) const; // returns the field that matches name, type, and type_count, otherwise returns nullptr.
