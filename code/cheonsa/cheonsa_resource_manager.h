...
namespace cheonsa
{
	...
	{
	private:
		core_list_c< resource_object_c * > _resource_list; // all of the resources currently instantiated.

		core_list_c< resource_object_c * > _load_queue; // main thread buffer of resources to add to the worker thread copy.

		platform_thread_c _worker_thread;
		platform_critical_section_c _worker_thread_critical_section;
		core_list_c< resource_object_c * > _worker_thread_load_queue;
		...
		void_c _refresh_resource( resource_object_c * resource ); // used by the scanner to check if a source file of a given resource was modified, and if so then it reloads the resource, otherwise it doesn't do anything.
		...
		static void_c _load_internal( resource_object_c * resource, data_stream_c * stream, string16_c const & file_path_absolute, boolean_c for_async_load_thread );
	...
	public:
		...
		resource_object_font_c * load_font( string16_c const & file_path_relative, boolean_c load_now = true );
