...
namespace cheonsa
{
	...
	reflection_property_c::reflection_property_c( reflection_class_c * reflection_class, string8_c const & name, string8_c const & description )
		...
		, _accessors()
	{
		cheonsa_assert( reflection_class != nullptr );
	...
	{
		cheonsa_assert( _type == data_type_e_none );
	...
	{
		cheonsa_assert( _type >= data_type_e_uint8 && _type <= data_type_e_float64 );
		cheonsa_assert( _type_count >= 1 && _type_count <= 4 );
		cheonsa_assert( default );
		uint8_c * target = _defaults_and_limits._uint8_default;
		ops::memory_copy( default, target, data_get_type_size( _type ) * _type_count );
	}

	void_c reflection_property_c::initialize_basic_limits( void_c * minimum, void_c * maximum )
	{
		cheonsa_assert( _type >= data_type_e_uint8 && _type <= data_type_e_float64 );
		cheonsa_assert( _type_count == 1 );
		cheonsa_assert( minimum != nullptr && maximum != nullptr );
		sint32_c type_size = data_get_type_size( _type );
		uint8_c * target = _defaults_and_limits._uint8_default;
	...
	{
		cheonsa_assert( _view == data_view_e_default );
		cheonsa_assert( _type == data_type_e_uint8 );
	...
	void_c reflection_property_c::initialize_basic_view_color()
	{
		...
		{
			cheonsa_assert( _defaults_and_limits._uint32_minimum == 0 && _defaults_and_limits._uint32_maximum == 0 );
		}
	...
	{
		cheonsa_assert( _view == data_view_e_default );
		cheonsa_assert( ( _type == data_type_e_float32 || _type == data_type_e_float64 ) && ( _type_count == 1 ) );
		if ( _type == data_type_e_float32 )
		{
			cheonsa_assert( _defaults_and_limits._float32_minimum < _defaults_and_limits._float32_maximum );
		}
		else if ( _type == data_type_e_float64 )
		{
			cheonsa_assert( _defaults_and_limits._float64_minimum < _defaults_and_limits._float64_maximum );
		}
	...
	{
		cheonsa_assert( _view == data_view_e_default );
		cheonsa_assert( _type == data_type_e_string8 || _type == data_type_e_string16 );
	...
	{
		cheonsa_assert( _view == data_view_e_default );
		cheonsa_assert( _type == data_type_e_float32 || _type == data_type_e_float64 );
		cheonsa_assert( _type_count == 3 || _type_count == 4 );
	...
	{
		cheonsa_assert( _view == data_view_e_default );
		cheonsa_assert( _type >= data_type_e_uint8 && _type <= data_type_e_sint64 );
		cheonsa_assert( _type_count == 1 );
		cheonsa_assert( enumeration );
	...
	void_c reflection_property_c::initialize_object( reflection_class_c const * _class, object_getter_f object_getter )
	{
		cheonsa_assert( _view == data_view_e_default );
		cheonsa_assert( _type == data_type_e_none );
		cheonsa_assert( _class );
		cheonsa_assert( object_getter );
	...
	void_c reflection_property_c::initialize_object_list( reflection_class_c const * _class, object_list_item_count_getter_f object_list_item_count_getter, object_list_item_getter_f object_list_item_getter, object_list_item_adder_f object_list_item_adder, object_list_item_remover_f object_list_item_remover, object_list_item_mover_f object_list_item_mover, object_list_item_sorter_f object_list_item_sorter )
	{
		...
		cheonsa_assert( _type == data_type_e_none );
		cheonsa_assert( _class );
		cheonsa_assert( object_list_item_count_getter );
		cheonsa_assert( object_list_item_getter );
		cheonsa_assert( object_list_item_adder );
		cheonsa_assert( object_list_item_remover );
