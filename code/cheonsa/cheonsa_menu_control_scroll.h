...
namespace cheonsa
{
	...
	// this scroll bar can be configured to be horizontal or vertical.
	class menu_control_scroll_c : public menu_control_c
	{
	public:
		static inline char8_c const * get_type_name_static() { return "scroll"; }
		virtual inline char8_c const * get_type_name() const override { return get_type_name_static(); }

	public:
		enum orientation_e
		{
			orientation_e_vertical,
			orientation_e_horizontal,
		};

		enum mode_e
		{
			mode_e_scroller, // scroll bar, default mode, if _fixed_grip_length > 0 then grip is of fixed length, otherwise grip length is calculated dynamically based on the ratio of ( _value_maximum - _value_minimum ) to _page_size.
			mode_e_scrubber, // scrub bar, grip is of a fixed length defined by _fixed_grip_length, and as long as the grip is dragged off of the center position then it will continue to move the value in that direction over time, and the grip will center itself when the user lets go of it.
		};
	...
	protected:
		...
		float32_c _fixed_thickness; // the thickness of the scroll bar in pixels.
		float32_c _fixed_grip_length; // if _mode is mode_e_slide_bar or mode_e_scrubber, then _grip_length will be set to this value.
		...
		virtual void_c _on_mouse_focus_gained() override;
	...
	public:
		...
		void_c set_orientation( orientation_e value );

		mode_e get_mode() const;
		void_c set_mode( mode_e value ); // every time the mode is changed, all of the other relevant properties need to be initialized.

		void_c update_visibility( menu_visibility_mode_e value );

		void_c set_value_range_and_page_size( float64_c value_minimum, float64_c value_maximum, float64_c page_size );
		...
		float32_c get_fixed_thickness() const;
		void_c set_fixed_thickness( float32_c value );

		float32_c get_fixed_grip_length() const;
		void_c set_fixed_grip_length( float32_c value );

		void_c inject_mouse_wheel_input( float32_c value );

	public:
		core_event_c< void_c, menu_control_scroll_c * > on_value_changed_preview; // occurs as the user drags the mouse.
		core_event_c< void_c, menu_control_scroll_c * > on_value_changed_commit; // occurs if the value is changed using the arrow keys or scroll wheel, or when the user lets go of the mouse drag.
		core_event_c< void_c, menu_control_scroll_c * > on_value_smoothed_changed; // occurs at most once per frame when the smooth value changes.
