...
namespace cheonsa
{
	...
	// it is its own thing, it does not work with bullet physics.
	...
	public:
		...
		{
		public:
			scene_matrix_c local_transform;
			scene_matrix_c world_target_transform;
			scene_matrix_c world_transform;
	...
	public:
		...
		//core_list_c< physics_body_c * > * body_list; // pointer to a list of bodies to collide against. this is usually owned by the model.

	public:
		physics_point_spring_strand_c();
		~physics_point_spring_strand_c();

		void_c update( space_transform_c & scene_transform, float32_c time_step, vector64x3_c gravity, vector64x3_c wind );
