	...
	boolean_c resource_file_material_map_c::_load( data_stream_c * stream )
	{
		...
		while ( sub_node )
		{
			if ( sub_node->get_value() == "material" )
			{
				...
				if ( attribute )
				{
					thing->material.pixel_shader = global_engine_instance.interfaces.video_renderer_shader_manager->load_pixel_shader( string16_c( attribute->get_value() ) );
				}
				...
				if ( attribute )
				{
					thing->material.textures[ 0 ] = global_engine_instance.interfaces.resource_manager->load_texture( string16_c( attribute->get_value() ) );
				}

				attribute = sub_node->find_attribute( "texture[1]" );
				if ( attribute )
				{
					thing->material.textures[ 1 ] = global_engine_instance.interfaces.resource_manager->load_texture( string16_c( attribute->get_value() ) );
				}

				attribute = sub_node->find_attribute( "texture[2]" );
				if ( attribute )
				{
					thing->material.textures[ 2 ] = global_engine_instance.interfaces.resource_manager->load_texture( string16_c( attribute->get_value() ) );
				}

				attribute = sub_node->find_attribute( "texture[3]" );
				if ( attribute )
				{
					thing->material.textures[ 3 ] = global_engine_instance.interfaces.resource_manager->load_texture( string16_c( attribute->get_value() ) );
				}
			}
