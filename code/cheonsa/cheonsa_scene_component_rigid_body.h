...
namespace cheonsa
{
	...
	// provides a rigid body object, for physics simulation.
	class scene_component_rigid_body_c
		: public scene_component_c
	{
	public:
		static inline uint8_c get_type_code_static() { return scene_component_type_e_rigid_body; }
	...
	private:
		...
		physics_rigid_body_c _rigid_body;
		...
		// copies the world space transform from the _scene_object to the _physics_rigid_body.
		...
		// bullet physics interoperability function.
		// copies the world space transform from the _physics_rigid_body to the _scene_object.
	...
	public:
		scene_component_rigid_body_c();
		virtual ~scene_component_rigid_body_c() override;
