...
namespace cheonsa
{
	...
	boolean_c script_instance_c::_evaluate_expression( core_list_c< script_value_c > & result, token_c * * token_array, sint32_c token_count )
	{
		...
		for ( sint32_c i = 0; i < token_list_a.get_length(); i++ )
		{
			...
			if ( token->type == script_instance_c::token_c::type_e_expression_begin )
			{
				...
				if ( last_token_was_name ) // making a function call
				{
					...
					if ( !global_engine_instance.interfaces.script_manager->call_function( this, last_token->string, sub_expression_result, function_result ) )
					{
						return false;
					}
				...
				else // resolved a sub expression to a value
				{
					...
					{
						_set_error( string16_c( mode_e_static, L"sub expression did not evaluate to exactly one value." ), token->original_line, token->original_column );
						return false;
					}
			...
			{
				_set_error( string16_c( mode_e_static, L"illegal keyword encountered in expression." ), token->original_line, token->original_column );
				return false;
			}
			else
			{
		...
		for ( sint32_c i = 0; i < token_list_b.get_length(); i++ )
		{
			...
			if ( token->type >= token_c::type_e_operator_times && token->type <= token_c::type_e_operator_modulo )
			{
				...
				{
					_set_error( string16_c( mode_e_static, L"times operator only works with number value types." ), token->original_line, token->original_column );
					return false;
				}
			}
			else
		...
		for ( sint32_c i = 0; i < token_list_a.get_length(); i++ )
		{
			...
			if ( token->type >= token_c::type_e_operator_plus && token->type <= token_c::type_e_operator_minus )
			{
				...
				{
					_set_error( string16_c( mode_e_static, L"plus operator only works with number value types." ), token->original_line, token->original_column );
					return false;
				}
			}
			else
		...
		for ( sint32_c i = 0; i < token_list_b.get_length(); i++ )
		{
			...
			if ( token->type >= token_c::type_e_operator_is_less_than && token->type <= token_c::type_e_operator_is_greater_than_or_equal_to )
			{
				...
				{
					_set_error( string16_c( mode_e_static,L"relational comparison operators only works with number value types." ), token->original_line, token->original_column );
					return false;
				}
			}
			else
		...
		for ( sint32_c i = token_list_a.get_length() - 1; i >= 0; i-- ) // scan from right to left
		{
			...
			if ( token->type == token_c::type_e_operator_assign )
			{
				...
				if ( i == 0 )
				{
					_set_error( string16_c( mode_e_static, L"coult not find left operand for assignment operator." ), token->original_line, token->original_column );
					return false;
				}
				i--;
				...
				{
					_set_error( string16_c( mode_e_static, L"left operand for assignment operator must be a name." ), token->original_line, token->original_column );
					return false;
				}
	...
	boolean_c script_instance_c::_resolve_l_r_operands( core_list_c< token_c * > & resolved_token_list, core_list_c< token_c * > & token_list, sint32_c operator_token_index, script_value_c * & l_operand, script_value_c * & r_operand )
	{
		...
		{
			_set_error( string16_c( mode_e_static, L"operator does not have operand." ), operator_token->original_line, operator_token->original_column );
			return false;
		}
		...
		{
			_set_error( string16_c( mode_e_static, L"operator does not have right operand." ), operator_token->original_line, operator_token->original_column );
			return false;
		}
		...
		{
			_set_error( string16_c( mode_e_static, L"left operand must be a value." ), l_token->original_line, l_token->original_column );
			return false;
		}
		...
		{
			_set_error( string16_c( mode_e_static, L"right operand must be a value." ), r_token->original_line, r_token->original_column );
			return false;
		}
	...
	boolean_c script_instance_c::_resolve_r_operand( core_list_c< token_c * > & token_list, sint32_c operator_token_index, script_value_c * & r_operand )
	{
		...
		if ( r_token_index >= token_list.get_length() )
		{
			_set_error( string16_c( mode_e_static, L"right operand was not found." ), operator_token->original_line, operator_token->original_column );
	...
	script_scope_c * script_instance_c::_resolve_scope( string8_c const & full_name, string8_c & name )
	{
		...
		ops::string8_split_at_delimiter( full_name, string8_c( mode_e_static, "." ), path_elements );
		...
		if ( path_elements.get_length() == 2 )
		{
			...
			return global_engine_instance.interfaces.script_manager->find_scope( path_elements[ 0 ] );
		}
		return nullptr;
	}
	...
	boolean_c script_instance_c::load( string16_c const & file_path, data_stream_c * file_stream )
	{
		...
		for ( sint32_c i = 0; i < _line_list.get_length(); i++ )
		{
			...
			if ( line.token_array[ 0 ].type == token_c::type_e_keyword_label )
			{
				...
				{
					_set_error( string16_c( mode_e_static, L"label line must contain exactly 2 tokens." ), i, 0 );
					return false;
				}

				if ( line.token_array[ 1 ].type != token_c::type_e_value_name )
				{
					_set_error( string16_c( mode_e_static, L"label must have a name." ), i, 0 );
					return false;
				}
	...
	boolean_c script_instance_c::update( float32_c time_step )
	{
		...
		while ( _line_index < _line_list.get_length() )
		{
			...
			{
				_set_error( string16_c( mode_e_static, L"bad indentation." ), line.token_array[ 0 ].original_line, line.token_array[ 0 ].original_column );
			...
			if ( line.token_array[ 0 ].type == token_c::type_e_keyword_else )
			{
				if ( line.resolved_if_group == false )
				{
					if ( line.token_array[ 1 ].type == token_c::type_e_keyword_if ) // else if
					{
						...
						{
							_set_error( string16_c( mode_e_static, L"if conditional expression did not resolve into a single value." ), line.token_array[ 0 ].original_line, line.token_array[ 0 ].original_column );
							_status = status_e_error;
							return false;
						}
